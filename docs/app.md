...
| 時刻表 | `ui/timetable/` | 南吉成 / 鳥取駅 / 宝木 のタブ。初期表示は今日で、今日 / 平日 / 土曜 / 日祝 を切り替え可能（日種別指定は該当する直近の日を引く）。今日の表示のみ現在時刻の位置へ自動スクロール。JR は ◆特定日のみ運転の便も **終電まで** 並べ、備考（「特定日のみ運転」）を添える | 鳥取駅タブは すべて / バスのみ / JRのみ で絞り込める
