...
| 設定 | `ui/settings/` | 所要時間・閾値（分）、往路/復路の時刻帯、前夜計算の基準時刻、出発時刻の表示時間帯、通知 ON/OFF、今日は休み、既定値に戻す |
...
- 初期表示は向きに近い側だけを拡大する（往路: 南吉成〜鳥取駅、復路: 宝木駅〜勤務先）。経路全体（約 18 km）を常に出すと自宅側の 2 点が重なるため。
  右上のボタンで全体表示と切り替える。現在地は近くにいるときだけ画面に収め、遠い（出張先など）ときは縁に点を置いて距離だけ示す。
- 経路線はバス区間を橙、JR 区間を青、勤務先までの徒歩を灰の破線で描く。現在地は青い点（波紋アニメーション）で、
...
3. 現在地が宝木駅（`model/Places.HOUGI_STATION`、概算座標）から 5 km 以内なら復路（職場を出る時刻）。
