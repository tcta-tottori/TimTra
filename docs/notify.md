...
| 何分前 | 段階 | アイコン | 文言 |
|---|---|---|---|
| 30 分 | EARLY | 歩く（`ic_walk`） | まだ余裕があります。歩いて 18 分ほどです |
| 20 分 | WALK | 歩く（`ic_walk`） | 今出れば普通に歩いて間に合います |
| 15 分 | FAST_WALK | 早歩き（`ic_walk_fast`） | 早歩きで向かってください（早歩きで 15 分） |

分数は実測（勤務先 → 宝木駅 は早歩きで 15 分）に合わせてあるので動かさない。
ただし 20 分前と 15 分前は 5 分しか離れていない。ドーズ中の `setExactAndAllowWhileIdle` は
1 アプリあたり 9 分に 1 回しか鳴らせないため、**バッテリー最適化の除外**（初回起動時に案内する）
が無いと 15 分前が数分遅れて届くことがある。設定画面の権限欄でここを確認できるようにしてある。

- 通勤通知と同じく **平日のみ**。土日祝・年末年始（JR の日種別）と「今日は休み」の日は予約しないし、
  予約済みでも鳴る瞬間にもう一度見て出さない（`isWorkday`）。位置が取れないときは出す側に倒すので、
  ここで落としておかないと休日に家で鳴ってしまう。
- 対象は設定「この時刻以降の電車が対象」（既定 17:00）以降に宝木を出る電車。列挙は core の
  `notify/TrainReminderPlanner`（純 Kotlin、テストあり）。同じ電車の 3 段階は同じ通知 ID で差し替える。
  宝木発の終電 23:14 も対象（逃すと帰れないので必ず鳴らす）。
  `JrService.irregular`（駅時刻表の ◆＝特定日のみ運転）を立てた便だけは対象から外すが、いまは該当便を同梱していない。
- 予約は通勤通知と同じく前夜と再計算時に AlarmManager へ入れる（`app/notify/TrainReminderScheduler`）。常駐しない。
- **鳴る瞬間の判定**: `NotificationAlarmReceiver` が位置を 1 回だけ取り（最大 6 秒）、
  勤務先から 1,500 m 以内（測位の誤差半径は差し引く）なら表示する。
  離れていると **確認できたときだけ** 表示せず、その日を `trainReminderOffDate` に記録して残りの予約を取り消す
  （「気高電機から離れた時点でその日の通知機能はオフ」）。
  位置が取れない・15 分より古い位置しか無いときは **表示する**。
  バックグラウンドの測位は「常に許可」が無い・ドーズ中・屋内などで簡単に失敗し、
  今朝の自宅の位置が返ることもある。それを「離れた」と扱うと、勤務先にいるのに通知が来ないうえ
  その日の残りまで消えてしまうため、確信が持てないときは出す側に倒す。
...
- 推奨する権限: 位置情報（前景）に加えて **「常に許可」（ACCESS_BACKGROUND_LOCATION）**。
  Android 10 以降、通知が鳴る瞬間の BroadcastReceiver はバックグラウンドなので、これが無いと位置が取れない。
  無くてもリマインダーは出る（上のとおり出す側に倒す）が、勤務先を離れても自動では止まらなくなる。
  設定画面に状態と導線を置いてある。
- 通知チャンネルは `train_reminder`（IMPORTANCE_DEFAULT）。通勤通知（`commute`）とは別に音量を調整できる。

## 復路の通知は宝木駅の発車だけ（2026-09-07）

復路（会社から帰るとき）の日次通知は `FIRST_LEG_DEPARTING`（「まもなく宝木発」、既定 3 分前）の 1 件だけにした
（`NotificationPlanner.plan` の `Bound.INBOUND` 分岐）。「職場を出る時刻」は勤務先リマインダー（30 / 20 / 15 分前）が担い、
鳥取駅でのバス発車（`APPROACHING_TRANSFER`）は駅や会社にいる帰りのタイミングでは不要なので出さない。往路の 4 件は従来どおり。
勤務先リマインダーの段階は実測（早歩きで 15 分）に合わせて 30 分前 = 余裕あり、20 分前 = 普通に歩いて間に合う、
15 分前 = 早歩きで、の文言とアイコンにした（`ReminderStage.EARLY / WALK / FAST_WALK`）。
