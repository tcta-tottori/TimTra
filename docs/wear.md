...
| 位置 | `location/WearLocationProvider` | 現在地を 1 回だけ取る。発車標の地点決めにしか使わない。**取れた位置は端末に控え**（SharedPreferences、90 分まで）、測位できないときはそれを使う |

## 現在地の許可（2026-09-20）

地点を「現在地から最寄り」で決めるには許可が要る。時計アプリは**開いたときに 1 回求める**
（`WearBoardScreen` の `RequestLocation`）。前面の許可が下りたら、続けて「常に許可」
（`ACCESS_BACKGROUND_LOCATION`）も 1 回だけ求める。

タイルとコンプリケーションはアプリが前面に無い状態で描かれるため、「常に許可」が無いと
**その場では測位できない**（Android 10 以降）。そこで `WearLocationProvider` は取れた位置を
端末に控えておき、測位できないときは 90 分以内の控えを使う。つまり:

| 状態 | タイルが出す地点 |
| --- | --- |
| 「常に許可」あり | その場で測位して最寄り |
| 前面のみ許可 | 直近 90 分に時計アプリを開いていれば、そのときの位置から最寄り |
| 許可なし / 控えも古い | 時刻帯で決めた向きの出発地（往路 = 南吉成、復路 = 宝木） |

どれで出ている地点かは 📍の色で分かる。**現在地から選べたときだけ青**、時刻帯で決めたときや
メニューで固定しているときは沈んだ色にする（ホームもタイルも同じ）。
メニューで地点を固定していると現在地より優先される（もう一度押すと自動に戻る）。
...
「/」は字見本に無いので組み見本から起こす。ただし組み見本の「9/1」は互いに近く、箱で切ると
隣の字の欠片とグローまで入ってしまう（実際、日にちの「1」に重なって字が欠けて見えていた）。
そこで **つながった形** で芯だけを抜き、グローは「:」と同じ作り方で足し直す（`make_slash`）。

重ね方も直してある。字画像にはグローが焼き込んであるので、そのまま上書きすると隣の字の
**暗いグローが手前の字を欠けさせる**。数字も日付も、重なりは明るいほうを残す
（`PorterDuff.Mode.LIGHTEN`。地が黒のときだけ使える手）。

並べ方は `DateArt.layout`。長さの単位は日にちの字の高さ、原点は日にちの字の左下で、
`DateGlyphs` の比率（字間・月の数字の大きさとベースライン・「/」の位置・曜日の大きさと右端）は
すべて組み見本の実測から出している。最後に組み上がりの **対角** を枠の 94% に合わせて拡大するので、
丸い枠でも切れずに目いっぱいの大きさになる。

合わせる相手は実際の日付ではなく、**その月でいちばん大きくなる組み**（幅の広い 2 桁の日にち 28 と、
曜日 7 つぶんを重ねた囲み）。実際の日付で合わせると、1 日のように横幅の狭い日だけ字が大きくなって
日ごとに大きさが変わってしまうため。
